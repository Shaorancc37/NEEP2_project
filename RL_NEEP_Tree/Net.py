...
class Net(nn.Module):
    ...
    def __init__(self,batch_size,input_size, hidden_size,num_layers,output_size, symbol_set , max_length, device):
        ...
        # 定义输入的embedding层
        self.input_embedding_layer = nn.Embedding(symbol_set.length, 8)
    ...
    def get_lstm_input(self,lstm_input):
        input_data = torch.clamp(lstm_input, min=0)
        input_embedding = self.input_embedding_layer(input_data).reshape(self.batch_size,-1)
    ...
    def forward(self):
        ...
        for i in range(self.max_length):
            input , father_list , now_nodepos_list  = getInput(tree_table,self.batch_size,self.input_size,self.symbol_set)
            # print(input)
            # print(father_list)
            # print(now_nodepos_list)

            x = self.get_lstm_input(input)
